...
/// adjust the autocorrelations using the given support (substract mu_theta and
/// clip to 0, normalize, and median filter) solve eq. (39), see Algorithm 4
template <typename T>
static void adjustAutocorrelations(img_t<T>& acProjectionsCorrected,
  ...
  for (int y = 0; y < h; y++) {
    ...
    for (int x = 0; x < w; x++) {
      if (x <= cut || x >= w - 1 - cut) {
        ...
        acProjectionsCorrected(x, y) =
            std::max(T(0.), acProjections(x, y) - cutVal);
      }
    }
  ...
  if (medianFilter) {
    circularMedianFilter(acProjectionsCorrected,
                         std::round(2. * std::sqrt(acProjections.h)));
  }
}

/// computed the autocorrelation of a signal up to a given window size
/// uses valid boundary condition
/// (this function differs from the computeAutocorrelation in the file
/// computeProjectionsAutocorrelation.cpp
///  by the fact that this one is adapted to small support of data)
template <typename T>
void computeAutocorrelationSmallSupport(std::vector<T>& out,
...
template <typename T>
static void reestimateKernelSupport(std::vector<int>& support,
                                    const img_t<T>& kernel,
  ...
  for (unsigned j = 0; j < angles.size(); j++) {
    ...
    // search for the first value exceeding the threshold on the right side of
    // the autocorrelation
    unsigned cursor = autocorrelation.size() - 1;
    while (cursor > autocorrelation.size() / 2 &&
           autocorrelation[cursor] < max * threshold)
      cursor--;
...
static void initialSupportEstimation(std::vector<int>& support,
  ...
  for (int j = 0; j < h; j++) {
    ...
    if (support[j] < currentMinimums[j]) {
      ...
      // propagate the constraint to other values (to both sides at the same
      // time)
      for (int yy = 1; yy <= h / 2; yy++) {
        // minimum value that is allowed at a distance 'yy' from the current
        // position
        T minimumValue = support[j] + yy * maxSlope;

        int prevRow = (j - yy + h) % h;
        currentMinimums[prevRow] =
            std::min(currentMinimums[prevRow], minimumValue);

        int nextRow = (j + yy) % h;
        currentMinimums[nextRow] =
            std::min(currentMinimums[nextRow], minimumValue);
      }
    }
  }
...
void estimateKernel(img_t<T>& kernel, const img_t<T>& img, int kernelSize,
  ...
  computeProjectionsAutocorrelation(acProjections, grey, angleSet,
                                    kernelSize * 2, opts.compensationFactor);
  ...
  for (int i = 0; i < opts.Nouter; i++) {
    ...
    adjustAutocorrelations(acProjectionsCorrected, acProjections, support,
                           opts.medianFilter);

    // compute the power spectrum from the autocorrelation
    reconstructPowerspectrum(powerSpectrum, acProjectionsCorrected, angleSet,
                             acRadius);
