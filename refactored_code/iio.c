...
static void *xmalloc(size_t size) {
  if (size == 0) fail("xmalloc: zero size");
  void *p = malloc(size);
  if (!p) {
    double sm = size / (0x100000 * 1.0);
    fail(
        "xmalloc: out of memory when requesting "
        "%zu bytes (%gMB)",  //:\"%s\"",
...
static void xfree(void *p) {
  if (!p) fail("thou shalt not free a null pointer!");
  free(p);
}
...
// internal API
static int iio_type_id(size_t sample_size, bool ieeefp_sample,
                       bool signed_sample) {
  if (ieeefp_sample) {
...
static void iio_type_unid(int *size, bool *ieefp, bool *signedness, int type) {
  ...
  *ieefp = (t == IIO_TYPE_HALF || t == IIO_TYPE_FLOAT || t == IIO_TYPE_DOUBLE ||
            t == IIO_TYPE_LONGDOUBLE);
...
static int iio_inttyp(const char *type_name) {
  ...
  char utyp[n + 1];
  utyp[n] = '\0';
  for (int i = 0; i < n; i++) utyp[i] = toupper(type_name[i]);
#define M(t) \
  if (!strcmp(utyp, #t)) return IIO_TYPE_##t
  M(INT8);
  M(UINT8);
  M(INT16);
  M(UINT16);
...
static void iio_image_fill(struct iio_image *x, int dimension, int *sizes,
...
static void iio_image_build_independent(struct iio_image *x, int dimension,
                                        int *sizes, int type,
                                        int pixel_dimension) {
  assert(dimension > 0);
  assert(dimension <= 4);
...
static void inplace_swap_pixels(struct iio_image *x, int i, int j, int a,
                                int b) {
  if (x->dimension != 2) fail("can only flip 2-dimensional images");
  int w = x->sizes[0];
...
static void inplace_flip_horizontal(struct iio_image *x) {
  ...
  int h = x->sizes[1];
  for (int j = 0; j < h; j++)
    for (int i = 0; i < w / 2; i++) inplace_swap_pixels(x, i, j, w - i - 1, j);
}
...
static void inplace_flip_vertical(struct iio_image *x) {
  ...
  int h = x->sizes[1];
  for (int j = 0; j < h / 2; j++)
    for (int i = 0; i < w; i++) inplace_swap_pixels(x, i, j, i, h - j - 1);
}

static void inplace_transpose(struct iio_image *x) {
  int w = x->sizes[0];
  int h = x->sizes[1];
  if (w != h) fail("rectangular inplace transpose not implemented");
  for (int i = 0; i < w; i++)
    for (int j = 0; j < i; j++) inplace_swap_pixels(x, i, j, j, i);
}

static void inplace_reorient(struct iio_image *x, int orientation) {
  int orx = orientation & 0xff;
  int ory = (orientation & 0xff00) / 0x100;
  if (isupper(orx)) inplace_flip_horizontal(x);
  if (isupper(ory)) inplace_flip_vertical(x);
  if (toupper(orx) > toupper(ory)) inplace_transpose(x);
}

static void inplace_trim(struct iio_image *x, int trim_left, int trim_bottom,
                         int trim_right, int trim_top) {
  ...
  for (int j = 0; j < nh; j++)
    for (int i = 0; i < nw; i++)
      memcpy(new_data + ps * (j * nw + i),
             old_data + ps * ((j + trim_top) * w + (i + trim_left)), ps);
  x->sizes[0] = nw;
  x->sizes[1] = nh;
  x->data = new_data;
  xfree(old_data);
...
static void iio_hacky_colorize(struct iio_image *x, int pd) {
  ...
  if (x->pixel_dimension != 1) fail("please, do not colorize color stuff");
...
static void *matrix_build(int w, int h, size_t n) {
  ...
  for (int i = 0; i < h; i++) *(void **)(r + i * p) = r + h * p + i * w * n;
  return r;
}
...
static void recover_broken_pixels_float(float *clear, float *broken, int n,
                                        int pd) {
...
static void recover_broken_pixels_uint8(uint8_t *clear, uint8_t *broken, int n,
                                        int pd) {
  FORL(pd) FORI(n) clear[pd * i + l] = broken[n * l + i];
}

static void repair_broken_pixels(void *clear, void *broken, int n, int pd,
                                 int sz) {
  char *c = clear;
  char *b = broken;
  FORL(pd) FORI(n) memcpy(c + sz * (pd * i + l), b + sz * (n * l + i), sz);
}
...
static void recover_broken_pixels_double(double *clear, double *broken, int n,
                                         int pd) {
  FORL(pd) FORI(n) clear[pd * i + l] = broken[n * l + i];
}
...
static int read_beheaded_png(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  (void)header;
...
static TIFF *tiffopen_fancy(const char *filename, char *mode) {
  ...
  *comma = '\0';
  ...
  if (!tif) return tif;
  for (int i = 0; i < index; i++) TIFFReadDirectory(tif);
...
static int read_whole_tiff(struct iio_image *x, const char *filename) {
  ...
  if (TIFFIsTiled(tif)) {
    ...
    if (bps < 8) fail("only byte-oriented tiles are supported (%d)", bps);
    int Bps = bps / 8;
    ...
    if (!broken) FORI(h) {
        ...
        if (bps < 8) {
          ...
          unpack_to_bytes_here(data + i * uscanline_size, buf, scanline_size,
                               bps);
          fmt_iio = IIO_TYPE_UINT8;
        } else {
      ...
      FORI(h) {
        FORJ(spp) {
          ...
          if (r < 0) fail("tiff bad %d/%d;%d", i, (int)h, j);
          memcpy(data + i * spp * sls + j * sls, buf, sls);
        }
        repair_broken_pixels_inplace(data + i * spp * sls, w, spp, bps / 8);
      }
    }
  }
  TIFFClose(tif);
...
static int read_beheaded_tiff(struct iio_image *x, FILE *fin, char *header,
                              int nheader) {
  if (global_variable_containing_the_name_of_the_last_opened_file) {
    int r = read_whole_tiff(
        x, global_variable_containing_the_name_of_the_last_opened_file);
...
static int read_qnm_numbers(float *data, FILE *f, int n, int m,
                            bool use_ascii) {
  if (use_ascii)
...
// 17 Q7 (ascii  3d nd           )
// 19 Q9 (binary 3d nd           )
static int read_beheaded_qnm(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  assert(nheader == 2);
  (void)header;
  (void)nheader;
...
static int read_beheaded_pcm(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  (void)header;
  assert(nheader == 2);
  int w, h;
...
static uint32_t rim_getint(FILE *f, bool swp) {
  ...
  assert(c < 256);
  assert(d < 256);
  uint32_t r = swp ? a * 0x1000000 + b * 0x10000 + c * 0x100 + d
...
static int read_beheaded_rim_ccimage(struct iio_image *x, FILE *f, bool swp) {
  ...
  if (pm_form == 0x8001) {  // ccimage
    ...
    if (r != nsamples)
      fail(
          "rim reader failed at reading %zu "
          "samples (got only %zu)\n",
          (size_t)nsamples, r);
...
static int read_beheaded_rim(struct iio_image *x, FILE *f, char *h, int nh) {
  assert(nh == 2);
  (void)nh;
  if (h[0] == 'I' && h[1] == 'R') return read_beheaded_rim_fimage(x, f, false);
  if (h[0] == 'R' && h[1] == 'I') return read_beheaded_rim_fimage(x, f, true);

  if (h[0] == 'M' && h[1] == 'I') return read_beheaded_rim_cimage(x, f, false);
  if (h[0] == 'I' && h[1] == 'M') return read_beheaded_rim_cimage(x, f, true);

  if (h[0] == 'W' && h[1] == 'E') return read_beheaded_rim_ccimage(x, f, false);
  if (h[0] == 'V' && h[1] == 'I') return read_beheaded_rim_ccimage(x, f, true);
  return 1;
}
...
// FLO reader                                                               {{{2
static int read_beheaded_flo(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  (void)header;
  (void)nheader;
...
// JUV reader                                                               {{{2
static int read_beheaded_juv(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  char buf[255];
...
static int read_beheaded_lum12(struct iio_image *x, FILE *f, char *header,
                               int nheader) {
  int w = *(uint32_t *)header;
  int h = *(uint32_t *)(header + 4);
  while (nheader++ < 11968) pick_char_for_sure(f);
...
static int read_beheaded_lum(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  if (header[8] == '1') return read_beheaded_lum12(x, f, header, nheader);
  int w = lum_pickshort(header + 2);
  int h = lum_pickshort(header + 6);
  while (nheader++ < 0xf94) pick_char_for_sure(f);
...
// temporary file, and read it from there.  This can be optimized in the future
// by recovering the original filename through hacks, in case it was not a pipe.
static int read_beheaded_exr(struct iio_image *x, FILE *fin, char *header,
                             int nheader) {
  if (global_variable_containing_the_name_of_the_last_opened_file) {
    int r = read_whole_exr(
        x, global_variable_containing_the_name_of_the_last_opened_file);
    if (r) fail("read whole tiff returned %d", r);
    return 0;
  }
...
// ASC reader                                                               {{{2
static int read_beheaded_asc(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  (void)nheader;
...
static int read_beheaded_pds(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  (void)header;
  (void)nheader;
  ...
  while ((n = getlinen(line, nmax, f)) >= 0 && cx++ < nmax) {
    ...
    if (!strcmp(key, "END_OBJECT") && !strcmp(value, object_id + 1)) break;
  }
  ...
  if (crop_left || crop_right) inplace_trim(x, crop_left, 0, crop_right, 0);
...
static int read_beheaded_dlm(struct iio_image *x, FILE *fin, char *header,
                             int nheader) {
  (void)x;
  (void)fin;
  (void)header;
...
static int read_beheaded_vrt(struct iio_image *x, FILE *fin, char *header,
                             int nheader) {
  (void)header;
  (void)nheader;
...
// FARBFELD reader                                                          {{{2
static int read_beheaded_ffd(struct iio_image *x, FILE *fin, char *header,
                             int nheader) {
  (void)header;
  (void)nheader;
  for (int i = 0; i < 4; i++) pick_char_for_sure(fin);
  int s[8];
  for (int i = 0; i < 8; i++) s[i] = pick_char_for_sure(fin);
...
static int parse_raw_binary_image_explicit(struct iio_image *x, void *data,
                                           size_t ndata, int w, int h, int pd,
  ...
  if (ndata < header_bytes + nsamples * ss) {
    fprintf(stderr, "WARNING:bad raw file size (%zu != %d + %zu)", ndata,
            header_bytes, nsamples * ss);
    // return 1;
  }
  int sizes[2] = {w, h};
  ...
  if (endianness) {
    if (ss == 2) switch_2endianness(x->data, nsamples);
    if (ss >= 4) switch_4endianness(x->data, nsamples);
  }
  return 0;
}
...
static int read_raw_named_image(struct iio_image *x, const char *filespec) {
  ...
  while (tok) {
    IIO_DEBUG("\ttoken = %s\n", tok);
    if (tok[1] == '@')
      field = raw_gfp(file_contents, file_size, 2 + tok, endianness);
    else
      field = atoi(1 + tok);
    IIO_DEBUG("\tfield=%d\n", field);
    switch (*tok) {
  ...
  if (offset < 0 || width < 0 || height < 0)
    fail(
        "could not determine width, height and offset"
        "(got %d,%d,%d)",
        width, height, offset);
  IIO_DEBUG("after estimation w=%d h=%d o=%d\n", width, height, offset);

  int used_data_size = offset + width * height * pd * ss;
  if (used_data_size > file_size) fail("raw file is not large enough");

  int r = parse_raw_binary_image_explicit(x, file_contents, file_size, width,
                                          height, pixel_dimension, offset,
                                          sample_type, brokenness, endianness);
  if (orientation) inplace_reorient(x, orientation);
  xfree(file_contents);
  return r;
}
...
static int read_beheaded_raw(struct iio_image *x, FILE *f, char *header,
                             int nheader) {
  (void)f;
  (void)header;
  (void)nheader;
...
int try_reading_file_with_libraw_4channels(const char *fname,
                                           struct iio_image *x);
#endif
...
static void iio_write_image_as_png(const char *filename, struct iio_image *x) {
  ...
  switch (normalize_type(x->type)) {
    ...
    case IIO_TYPE_UINT8:
      ...
      break;
    default:
      fail("can not yet save samples of type %s as PNG", iio_strtyp(x->type));
  }
  assert(bit_depth > 0);
  ...
  switch (x->pixel_dimension) {
    ...
    case 4:
      ...
      break;
    default:
      fail("can not save %d-dimensional samples as PNG", x->pixel_dimension);
  }
  ...
  png_set_IHDR(pp, pi, width, height, bit_depth, color_type, PNG_INTERLACE_NONE,
...
static void iio_write_image_as_tiff(const char *filename, struct iio_image *x) {
  if (x->dimension != 2) fail("only 2d images can be saved as TIFFs");
  ...
  switch (x->type) {
    ...
    case IIO_TYPE_UINT32:
      ...
      break;
    default:
      fail("can not save samples of type %s on tiff file", iio_strtyp(x->type));
  }
...
static void iio_write_image_as_ppm(const char *filename, struct iio_image *x) {
  ...
  float scale = 255;
  fprintf(f, "P%c\n%d %d\n%g\n", dimchar, w, h, scale);
  for (int i = 0; i < w * h * pd; i++) fprintf(f, "%d\n", (int)t[i]);
  xfclose(f);
}
...
static void iio_write_image_as_asc(const char *filename, struct iio_image *x) {
  if (x->type == IIO_TYPE_FLOAT) {
    ...
    for (int i = 0; i < w * h * pd; i++) fprintf(f, "%.9g\n", t[i]);
    xfree(t);
    xfclose(f);
...
static void iio_write_image_as_rim_fimage(const char *fname,
                                          struct iio_image *x) {
...
static void iio_write_image_as_rim_cimage(const char *fname,
                                          struct iio_image *x) {
...
static char add_to_header_buffer(FILE *f, uint8_t *buf, int *nbuf, int bufmax) {
  int c = pick_char_for_sure(f);
  if (*nbuf >= bufmax) fail("buffer header too small (%d)", bufmax);
  buf[*nbuf] = c;  // iw810
...
static void line_to_header_buffer(FILE *f, uint8_t *buf, int *nbuf,
                                  int bufmax) {
  while (*nbuf < bufmax) {
    int c = pick_char_for_sure(f);
    buf[*nbuf] = c;
...
static int guess_format(FILE *f, char *buf, int *nbuf, int bufmax) {
  ...
  *nbuf = 0;
  ...
  if (b[0] == 'P' || b[0] == 'Q')
    if (b[1] >= '1' && b[1] <= '9') return IIO_FORMAT_QNM;

  if (b[0] == 'P' && (b[1] == 'F' || b[1] == 'f')) return IIO_FORMAT_PFM;
...
#ifdef I_CAN_HAS_LIBPNG
  if (b[1] == 'P' && b[2] == 'N' && b[3] == 'G') return IIO_FORMAT_PNG;
#endif  // I_CAN_HAS_LIBPNG
  ...
  if (b[0] == '#' && b[1] == 'U' && b[2] == 'V') return IIO_FORMAT_JUV;
  ...
  if (b[0] == 0xff && b[1] == 0xd8 && b[2] == 0xff) {
    if (b[3] == 0xe0 && b[6] == 'J' && b[7] == 'F') return IIO_FORMAT_JPEG;
    if (b[3] == 0xe1 && b[6] == 'E' && b[7] == 'x') return IIO_FORMAT_JPEG;
  ...
  while (*nbuf < bufmax) add_to_header_buffer(f, b, nbuf, bufmax);

  bool buffer_statistics_agree_with_csv(uint8_t *, int);
  if (buffer_statistics_agree_with_csv(b, bufmax)) return IIO_FORMAT_CSV;

  bool buffer_statistics_agree_with_dlm(uint8_t *, int);
  if (buffer_statistics_agree_with_dlm(b, bufmax)) return IIO_FORMAT_DLM;

  if (getenv("IIO_RAW")) return IIO_FORMAT_RAW;
...
static int read_beheaded_image(struct iio_image *x, FILE *f, char *h, int hn,
                               int fmt) {
  IIO_DEBUG("rbi fmt = %d\n", fmt);
...
static int read_image(struct iio_image *x, const char *fname) {
  ...
  if (fname == strstr(fname, "zero:")) {
    ...
    if (3 == sscanf(fname + 5, "%dx%d,%d", s, s + 1, &pd))
      ...
      fail("bad semantical name \"%s\"", fname);
    iio_image_build_independent(x, 2, s, IIO_TYPE_CHAR, pd);
    for (int i = 0; i < *s * s[1] * pd; i++) ((char *)x->data)[i] = 0;
    return 0;
  }
  if (fname == strstr(fname, "one:")) {
    ...
    if (3 == sscanf(fname + 4, "%dx%d,%d", s, s + 1, &pd))
      ...
      fail("bad semantical name \"%s\"", fname);
    iio_image_build_independent(x, 2, s, IIO_TYPE_CHAR, pd);
    for (int i = 0; i < *s * s[1] * pd; i++) ((char *)x->data)[i] = 1;
    return 0;
  }
  if (fname == strstr(fname, "constant:")) {
    ...
    if (3 == sscanf(fname + 9, "%g:%dx%d", &value, s, s + 1))
      ...
      fail("bad semantical name \"%s\"", fname);
    iio_image_build_independent(x, 2, s, IIO_TYPE_CHAR, pd);
    for (int i = 0; i < *s * s[1] * pd; i++) ((char *)x->data)[i] = value;
    return 0;
  }
...
//// 2D only
// static
// void *iio_read_image(const char *fname, int *w, int *h, int
// desired_sample_type)
//{
...
// API 2D
double *iio_read_image_double_split(const char *fname, int *w, int *h,
                                    int *pd) {
...
uint8_t (*iio_read_image_uint8_rgb(const char *fname, int *w, int *h))[3] {
  ...
  if (x->pixel_dimension != 3) fail("non-color image");
  *w = x->sizes[0];
  *h = x->sizes[1];
  iio_convert_samples(x, IIO_TYPE_UINT8);
  return x->data;
...
// API 2D
uint8_t (**iio_read_image_uint8_matrix_rgb(const char *fnam, int *w,
                                           int *h))[3] {
  struct iio_image x[1];
...
// API 2D
uint8_t ***iio_read_image_uint8_matrix_vec(const char *fname, int *w, int *h,
                                           int *pd) {
  struct iio_image x[1];
  int r = read_image(x, fname);
  if (r) return rfail("could not read image");
...
// API 2D
void *iio_read_image_float_matrix_vec(const char *fname, int *w, int *h,
                                      int *pd) {
  struct iio_image x[1];
  int r = read_image(x, fname);
  if (r) return rfail("could not read image");
...
uint8_t **iio_read_image_uint8_matrix(const char *fname, int *w, int *h) {
  ...
  if (x->pixel_dimension == 3) iio_hacky_uncolorize(x);
  if (x->pixel_dimension != 1) fail("non-scalar image");
  *w = x->sizes[0];
  *h = x->sizes[1];
  iio_convert_samples(x, IIO_TYPE_UINT8);
...
float **iio_read_image_float_matrix(const char *fname, int *w, int *h) {
  ...
  if (x->pixel_dimension == 3) iio_hacky_uncolorize(x);
  if (x->pixel_dimension != 1) return rfail("non-scalar image");
  *w = x->sizes[0];
  *h = x->sizes[1];
  iio_convert_samples(x, IIO_TYPE_FLOAT);
...
// API nd general
void *iio_read_nd_image_as_stored(char *fname, int *dimension, int *sizes,
...
// API nd general
void *iio_read_nd_image_as_desired(char *fname, int *dimension, int *sizes,
  ...
  int r = read_image(x, fname);
  if (r) return rfail("so much fail");
  int desired_type = iio_type_id(desired_sample_size, desired_ieeefp_samples,
...
float *iio_read_image_float(const char *fname, int *w, int *h) {
  ...
  if (x->pixel_dimension == 3) iio_hacky_uncolorize(x);
  if (x->pixel_dimension == 4) iio_hacky_uncolorizea(x);
  if (x->pixel_dimension != 1) return rfail("non-scalarizable image");
  *w = x->sizes[0];
  *h = x->sizes[1];
  iio_convert_samples(x, IIO_TYPE_FLOAT);
  return x->data;
...
static bool these_floats_are_actually_bytes(float *t, int n) {
  ...
  FORI(n)
  if (!this_float_is_actually_a_byte(t[i])) return false;
  return true;
}
...
static void iio_write_image_default(const char *filename, struct iio_image *x) {
...
#endif  // I_CAN_HAS_LIBTIFF
  if (typ != IIO_TYPE_DOUBLE && typ != IIO_TYPE_FLOAT &&
      typ != IIO_TYPE_UINT8 && typ != IIO_TYPE_INT16 && typ != IIO_TYPE_INT8 &&
      typ != IIO_TYPE_UINT32 && typ != IIO_TYPE_UINT16)
...
#ifdef I_CAN_HAS_LIBTIFF
  if (true) {
    if (false || string_suffix(filename, ".tiff") ||
        string_suffix(filename, ".tif") || string_suffix(filename, ".TIFF") ||
  ...
  if (true) {
    if (false || string_suffix(filename, ".png") ||
  ...
  if (x->pixel_dimension == 1 && typ == IIO_TYPE_FLOAT) {
    int m = these_floats_are_actually_bytes(x->data, x->sizes[0] * x->sizes[1])
                ? 255
                : 65535;
    ...
    int m =
        these_floats_are_actually_bytes(x->data, 3 * x->sizes[0] * x->sizes[1])
            ? 255
            : 65535;
    float *data = x->data;
    fprintf(f, "P3\n%d %d\n%d\n", x->sizes[0], x->sizes[1], m);
    FORI(3 * x->sizes[0] * x->sizes[1]) { fprintf(f, "%g\n", data[i]); }
...
void iio_write_image_uint8_matrix(char *filename, uint8_t **data, int w,
                                  int h) {
  struct iio_image x[1];
  x->dimension = 2;
  x->sizes[0] = w;
...
void iio_write_image_float_vec(char *filename, float *data, int w, int h,
                               int pd) {
  struct iio_image x[1];
  x->dimension = 2;
  x->sizes[0] = w;
...
void iio_write_image_float_split(char *filename, float *data, int w, int h,
                                 int pd) {
...
void iio_write_image_int_vec(char *filename, int *data, int w, int h, int pd) {
  struct iio_image x[1];
  x->dimension = 2;
  x->sizes[0] = w;
  x->sizes[1] = h;
...
void iio_write_image_double_vec(char *filename, double *data, int w, int h,
                                int pd) {
  struct iio_image x[1];
  x->dimension = 2;
  x->sizes[0] = w;
...
void iio_write_image_double_split(char *filename, double *data, int w, int h,
                                  int pd) {
...
void iio_write_image_uint8_vec(char *filename, uint8_t *data, int w, int h,
                               int pd) {
  struct iio_image x[1];
  x->dimension = 2;
  x->sizes[0] = w;
...
void iio_write_image_uint8_split(char *filename, uint8_t *data, int w, int h,
                                 int pd) {
...
void iio_write_image_uint16_vec(char *filename, uint16_t *data, int w, int h,
                                int pd) {
  struct iio_image x[1];
  x->dimension = 2;
  x->sizes[0] = w;
...
void iio_save_image_float_vec(char *filename, float *x, int w, int h, int pd) {
  return iio_write_image_float_vec(filename, x, w, h, pd);
}
void iio_save_image_float_split(char *filename, float *x, int w, int h,
                                int pd) {
  return iio_write_image_float_split(filename, x, w, h, pd);
}
void iio_save_image_double_vec(char *filename, double *x, int w, int h,
                               int pd) {
  return iio_write_image_double_vec(filename, x, w, h, pd);
}
void iio_save_image_float(char *filename, float *x, int w, int h) {
  return iio_write_image_float(filename, x, w, h);
}
void iio_save_image_double(char *filename, double *x, int w, int h) {
  return iio_write_image_double(filename, x, w, h);
}
void iio_save_image_int(char *filename, int *x, int w, int h) {
  return iio_write_image_int(filename, x, w, h);
}
void iio_save_image_int_vec(char *filename, int *x, int w, int h, int pd) {
  return iio_write_image_int_vec(filename, x, w, h, pd);
}
void iio_save_image_uint8_vec(char *filename, uint8_t *x, int w, int h,
                              int pd) {
  return iio_write_image_uint8_vec(filename, x, w, h, pd);
}
void iio_save_image_uint16_vec(char *filename, uint16_t *x, int w, int h,
                               int pd) {
  return iio_write_image_uint16_vec(filename, x, w, h, pd);
}
void iio_save_image_uint8_matrix_rgb(char *f, unsigned char (**x)[3], int w,
                                     int h) {
  return iio_write_image_uint8_matrix_rgb(f, x, w, h);
}
void iio_save_image_uint8_matrix(char *f, unsigned char **x, int w, int h) {
  return iio_write_image_uint8_matrix(f, x, w, h);
}
