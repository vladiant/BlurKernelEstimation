...
int AllocImageObj(image *f, int Width, int Height, int NumChannels) {
  ...
  if (!(f->Data =
            (num *)malloc(sizeof(num) * (((size_t)Width) * ((size_t)Height)) *
                          NumChannels))) {
    *f = NullImage;
    return 0;
  }
...
int ReadImageObj(image *f, const char *FileName) {
  if (!f || !(f->Data = (num *)ReadImage(
                  &f->Width, &f->Height, FileName,
...
int WriteImageObj(image f, const char *FileName, int JpegQuality) {
  ...
  switch (f.NumChannels) {
    ...
    case 4:
      return WriteImage(f.Data, f.Width, f.Height, FileName,
                        IMAGEIO_NUM | IMAGEIO_RGBA | IMAGEIO_PLANAR,
                        JpegQuality);
    default:
      return 0;
  }
...
int IsGrayscale(num *Data, int Width, int Height) {
  ...
  for (n = 0; n < NumPixels; n++)
    if (Red[n] != Green[n] || Red[n] != Blue[n]) return 0;
 ...
 */
int GetStrToken(char *Token, const char *Start, int MaxLength,
                const char *Delim) {
...
int ParseDouble(double *Num, const char *String) {
  ...
  while (isdigit(c = String[++i])) Accum = 10 * Accum + (c - '0');
  ...
  if (c == 'e' || c == 'E') /* There is an exponent */
  {
    ...
    if (isdigit(c = String[i])) {
      ...
      while (isdigit(c = String[++i])) Exponent = 10 * Exponent + (c - '0');
...
@endcode
 */
int CliParseArglist(const char **Param, const char **Value, char *TokenBuf,
                    int MaxLength, int k, const char *Start, int argc,
                    const char *argv[], const char *Delimiters) {
  int TokLen, kread = k;
...
int ReadMatrixFromTextFile(image *f, const char *FileName) {
  ...
  *f = NullImage;
  ...
  if (!(Dest = (num *)malloc(sizeof(num) * DestCapacity))) goto Catch;
