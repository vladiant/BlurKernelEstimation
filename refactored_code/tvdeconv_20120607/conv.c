...
void SampledConv1D(float *Dest, int DestStride, const float *Src, int SrcStride,
  ...
  const int EndInterior = (Filter.Delay < 0) ? (N + Filter.Delay - 1) : (N - 1);
  const float *SrcN, *SrcK;
  float Accum;
  int n, k;

  if (nEnd < nStart || nStep <= 0 || N <= 0) return;

  /* Handle the left boundary */
  for (n = nStart; n < StartInterior; n += nStep, Dest += DestStride) {
    for (k = 0, Accum = 0; k < Filter.Length; k++)
      Accum +=
          Filter.Coeff[k] * Boundary(Src, SrcStride, N, n - Filter.Delay - k);
  ...
  for (; n <= EndInterior;
       n += nStep, SrcN += SrcStrideStep, Dest += DestStride) {
    Accum = 0;
    SrcK = SrcN;
    k = Filter.Length;
...
void SeparableConv2D(float *Dest, float *Buffer, const float *Src,
  ...
  for (Channel = 0; Channel < NumChannels; Channel++) {
    ...
    for (i = 0; i < Height; i++)
      Conv1D(Buffer + Width * i, 1, Src + Width * i, 1, FilterX, Boundary,
             Width);

    /* Filter Buffer vertically and store the result in Dest */
    for (i = 0; i < Width; i++)
      Conv1D(Dest + i, Width, Buffer + i, Width, FilterY, Boundary, Height);
...
int IsNullFilter(filter Filter) { return (Filter.Coeff == NULL) ? 1 : 0; }
...
filter GaussianFilter(double Sigma, int R) {
  ...
  if (!IsNullFilter(Filter)) {
    if (Sigma == 0)
      ...
      for (r = -R; r <= R; r++) Filter.Coeff[R + r] /= Sum;
    }
  }
...
boundaryext GetBoundaryExt(const char *Boundary) {
  ...
  else if (!strcmp(Boundary, "sym") || !strcmp(Boundary, "symh") ||
           !strcmp(Boundary, "hsym"))
    return SymhExtension;
  else if (!strcmp(Boundary, "symw") || !strcmp(Boundary, "wsym"))
    return SymwExtension;
  else if (!strcmp(Boundary, "asym") || !strcmp(Boundary, "asymh") ||
           !strcmp(Boundary, "hasym"))
