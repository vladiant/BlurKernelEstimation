...
#define ROUNDCLAMPF(x) \
  ((x < 0.0f) ? 0 : ((x > 1.0f) ? 255 : (uint8_t)(255.0f * (x) + 0.5f)))
#define ROUNDCLAMP(x) \
  ((x < 0.0) ? 0 : ((x > 1.0) ? 255 : (uint8_t)(255.0 * (x) + 0.5)))
...
static int StringEndsWith(const char *String, const char *Suffix) {
  ...
  if (StringLength < SuffixLength) return 0;

  String += StringLength - SuffixLength;

  for (i = 0; i < SuffixLength; i++)
    if (tolower(String[i]) != tolower(Suffix[i])) return 0;
...
static void FillImage(uint32_t *Image, int Width, int Height, uint32_t Color) {
  ...
  if (Image)
    for (y = 0; y < Height; y++, Image += Width)
      for (x = 0; x < Width; x++) Image[x] = Color;
}
  ...
  for (y = 0; y < Height; y++) {
    for (x = 0; x < Width; x++) {
      ...
      for (i = 0; i < *NumColors; i++)
        if (Pixel == Palette[i]) break;

      if (i == *NumColors) {
        if (i < MaxColors) { /* Add new color to Palette */
          Palette[i] = Pixel;
          (*NumColors)++;
...
static int ReadBmp1Bit(uint32_t *Image, int Width, int Height, FILE *File,
...
static int ReadBmp4Bit(uint32_t *Image, int Width, int Height, FILE *File,
                       const uint32_t *Palette) {
  ...
  for (y = Height; y; y--, Image -= Width) {
    ...
    for (x = 0; x < Width;) {
      ...
      if (x < Width) Image[x++] = Palette[Code & 0x0F];
    }
...
static int ReadBmp4BitRle(uint32_t *Image, int Width, int Height, FILE *File,
                          const uint32_t *Palette) {
  int x, y, dy, k;
    ...
    if (!Count) { /* Count = 0 is the escape code */
      switch (Value) {
        ...
        default: /* Read a run of uncompressed data (Value = length of run) */
          ...
          if (((Count + 1) / 2) & 1) getc(File); /* Padding for word align */
      }
...
static int ReadBmp8Bit(uint32_t *Image, int Width, int Height, FILE *File,
                       const uint32_t *Palette) {
  ...
  for (y = Height; y; y--, Image -= Width) {
    ...
    for (x = 0; x < Width; x++) Image[x] = Palette[getc(File) & 0xFF];
...
static int ReadBmp8BitRle(uint32_t *Image, int Width, int Height, FILE *File,
                          const uint32_t *Palette) {
  int x, y, dy, k;
  unsigned Count, Value;
  uint32_t Color;
    ...
    if (!Count) { /* Count = 0 is the escape code */
      switch (Value) {
        ...
        default: /* Read a run of uncompressed data (Value = length of run) */
          ...
          if (Count & 1) getc(File); /* Padding for word align */
      }
...
static int ReadBmp16Bit(uint32_t *Image, int Width, int Height, FILE *File,
  ...
  for (y = Height; y; y--, ImagePtr -= Width) {
    ...
    for (x = 0; x < Width; x += 4) {
      ...
      /* By the Windows 4.x BMP specification, color component masks must be
      contiguous [http://www.fileformat.info/format/bmp/egff.htm].  So we can
      decode the bitfields by bitwise AND with the mask and applying a
      bitshift.*/
      ImagePtr[x + 3] = ((Code & AlphaMask) >> AlphaRightShift)
                        << AlphaLeftShift;
      ImagePtr[x + 2] = ((Code & BlueMask) >> BlueRightShift) << BlueLeftShift;
      ImagePtr[x + 1] = ((Code & GreenMask) >> GreenRightShift)
                        << GreenLeftShift;
...
static int ReadBmp(uint32_t **Image, int *Width, int *Height, FILE *File) {
  ...
  *Width = *Height = 0;
  ...
  if (feof(File) || NumPlanes != 1 || Compression > 3) goto Catch;

  /* Allocate the image data */
  if (!(*Image = (uint32_t *)Malloc(sizeof(uint32_t) * ((long int)*Width) *
                                    ((long int)*Height))))
    goto Catch;
  ...
  if (BitsPerPixel <= 8) {
    ...
    if (!NumColors) NumColors = 1 << BitsPerPixel;

    if (!(Palette = (uint32_t *)Malloc(sizeof(uint32_t) * 256))) goto Catch;
    ...
    for (i = 0, PalettePtr = (uint8_t *)Palette; i < NumColors; i++) {
      ...
      if (!Os2Bmp) getc(File); /* Skip extra byte (for non-OS/2 bitmaps) */
    }
    ...
    case 0: /* Uncompressed data */
      switch (BitsPerPixel) {
        ...
        case 16: /* Read 16-bit data */
          Success = ReadBmp16Bit(*Image, *Width, *Height, File, 0x001F << 10,
                                 0x001F << 5, 0x0001F, 0);
          break;
        case 32: /* Read 32-bit BGRA image data */
          Success = ReadBmp32Bit(*Image, *Width, *Height, File, 0x00FF0000,
    ...
    case 3: /* Bitfields data */
      switch (BitsPerPixel) {
        case 16: /* Read 16-bit bitfields data */
          Success = ReadBmp16Bit(*Image, *Width, *Height, File, RedMask,
                                 GreenMask, BlueMask, AlphaMask);
          break;
        case 32: /* Read 32-bit bitfields data */
          Success = ReadBmp32Bit(*Image, *Width, *Height, File, RedMask,
                                 GreenMask, BlueMask, AlphaMask);
          break;
      }
      break;
  ...
  if (!Success) ErrorMessage("Error reading BMP data.\n");

Catch: /* There was a problem, clean up and exit */
  if (Palette) Free(Palette);

  if (!Success && *Image) Free(*Image);
...
static int WriteBmp(const uint32_t *Image, int Width, int Height, FILE *File) {
  ...
  Palette =
      GetImagePalette(&NumColors, &UseColor, &UseAlpha, Image, Width, Height);
  ...
  if (UsePalette) { /* Write the Palette */
    for (i = 0; i < NumColors; i++) {
      Pixel = Palette[i];
  ...
  for (y = Height; y; y--, ImagePtr -= Width) {
    if (UsePalette) { /* 8-bit palette image data */
      for (x = 0; x < Width; x += 4) {
...
static int ReadJpeg(uint32_t **Image, int *Width, int *Height, FILE *File) {
  ...
  *Height = (int)cinfo.output_height;
  ...
  if (!(*Image = (uint32_t *)Malloc(sizeof(uint32_t) * ((size_t)*Width) *
                                    ((size_t)*Height)))) {
    jpeg_abort_decompress(&cinfo);
    goto Catch;
  }
  ...
  Buffer =
      (*cinfo.mem->alloc_sarray)((j_common_ptr)&cinfo, JPOOL_IMAGE, RowSize, 1);
 ...
 *       is strange.)
 */
static int WriteJpeg(const uint32_t *Image, int Width, int Height, FILE *File,
                     int Quality) {
  ...
  if (!(Buffer = (uint8_t *)Malloc(RowSize))) goto Catch;
...
static int ReadPng(uint32_t **Image, int *Width, int *Height, FILE *File) {
  ...
  *Width = *Height = 0;
  ...
  if (fread(Header, 1, 8, File) != 8 || png_sig_cmp(Header, 0, 8)) return 0;

  /* Read the info header */
  if (!(Png =
            png_create_read_struct(PNG_LIBPNG_VER_STRING, NULL, NULL, NULL)) ||
      !(Info = png_create_info_struct(Png))) {
    if (Png) png_destroy_read_struct(&Png, (png_infopp)NULL, (png_infopp)NULL);
  ...
  *Height = (int)PngHeight;
  ...
  if (!(*Image = (uint32_t *)Malloc(sizeof(uint32_t) * ((size_t)*Width) *
                                    ((size_t)*Height))) ||
      !(RowPointers = (png_bytep *)Malloc(sizeof(png_bytep) * PngHeight)))
    goto Catch;
...
static int WritePng(const uint32_t *Image, int Width, int Height, FILE *File) {
  ...
  if (setjmp(png_jmpbuf(
          Png))) { /* If this code is reached, libpng has signaled an error. */
    goto Catch;
  }
  ...
  png_set_IHDR(Png, Info, Width, Height, 8, PngColorType, PNG_INTERLACE_NONE,
  ...
  if (PngColorType == PNG_COLOR_TYPE_PALETTE) {
    ...
    if (UseAlpha) png_set_tRNS(Png, Info, PngTrans, NumColors, NULL);
  }
...
static int ReadTiff(uint32_t **Image, int *Width, int *Height,
  ...
  *Height = (int)ImageHeight;
  ...
  if (!(*Image =
            (uint32_t *)Malloc(sizeof(uint32_t) * ImageWidth * ImageHeight)))
    goto Catch;
...
static void *ConvertToFormat(uint32_t *Src, int Width, int Height,
  ...
  const int NumChannels = (Format & IMAGEIO_GRAYSCALE)
                              ? 1
                              : ((Format & IMAGEIO_STRIP_ALPHA) ? 3 : 4);
  ...
  switch (Format & (IMAGEIO_U8 | IMAGEIO_SINGLE | IMAGEIO_DOUBLE)) {
    case IMAGEIO_U8: /* Destination type is uint8_t */
      if (!(DestU8 =
                (uint8_t *)Malloc(sizeof(uint8_t) * NumChannels * NumPixels)))
        return NULL;
        ...
        case 1: /* Convert RGBA U8 to grayscale double */
          for (y = 0; y < Height; y++, Src += Width)
            for (x = 0, i = RowStride * y; x < Width; x++, i += PixelStride) {
              Pixel = Src[x];
              DestD[i] =
                  1.172549019607843070675535e-3 * ((uint8_t *)&Pixel)[0] +
...
static uint32_t *ConvertFromFormat(void *Src, int Width, int Height,
  ...
  if (!(Dest = (uint8_t *)Malloc(sizeof(uint32_t) * NumPixels))) return NULL;
      ...
      switch (NumChannels) {
        case 1: /* Convert grayscale U8 to RGBA U8 */
          for (y = 0; y < Height; y++, DestPtr += 4 * Width)
            for (x = 0, i = RowStride * y; x < Width; x++, i += PixelStride) {
              DestPtr[4 * x] = DestPtr[4 * x + 1] = DestPtr[4 * x + 2] =
                  SrcU8[i];
              DestPtr[4 * x + 3] = 255;
            }
          break;
...
* range 0 to 1.
*/
void *ReadImage(int *Width, int *Height, const char *FileName,
                unsigned Format) {
  void *Image = NULL;
  ...
  if (!strcmp(Type, "BMP")) {
    ...
    fprintf(stderr, "\nSorry, only " READIMAGE_FORMATS_SUPPORTED
 ...
 */
int WriteImage(void *Image, int Width, int Height, const char *FileName,
               unsigned Format, int Quality) {
  FILE *File;
  uint32_t *ImageU8;
  ...
  if (StringEndsWith(FileName, ".bmp"))
    ...
    ErrorMessage("Sorry, only " WRITEIMAGE_FORMATS_SUPPORTED
