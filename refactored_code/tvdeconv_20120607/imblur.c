...
static void PrintHelpMessage() {
  puts(
      ...
      "where \"input\" and \"output\" are " READIMAGE_FORMATS_SUPPORTED
      " files.\n");
  puts("Parameters");
...
numcomplex *ComputePaddedDFT(num *PadTemp, int PadWidth, int PadHeight,
  ...
  /* Allocate memory */
  if (!(Dest =
            (numcomplex *)FFT(malloc)(sizeof(numcomplex) * TransNumPixels))) {
    fprintf(stderr, "Memory allocation failed.\n");
    return NULL;
  }

  /* Create plan for DFT transform of PadTemp */
  if (!(Plan = FFT(plan_dft_r2c_2d)(PadHeight, PadWidth, PadTemp, Dest,
  ...
  if (IsKernelFlag) /* Do zero-padding extrapolation for the kernel */
  {
    ...
    for (y = 0; y < SrcWidth; y++)
      for (x = 0; x < SrcHeight; x++) PadTemp[x + ((long)PadWidth) * y] = 0;

    for (y = 0; y < SrcHeight; y++)
      for (x = 0; x < SrcWidth; x++)
        PadTemp[((x < SrcHalfWidth) ? (PadWidth - SrcHalfWidth + x)
                                    : (x - SrcHalfWidth)) +
                ((long)PadWidth) * ((y < SrcHalfHeight)
                                        ? (PadHeight - SrcHalfHeight + y)
                                        : (y - SrcHalfHeight))] =
            Src[x + SrcWidth * y];
  } else /* Whole-sample symmetric extrapolation for the image */
    for (y = 0; y < PadHeight; y++)
      for (x = 0; x < PadWidth; x++)
        PadTemp[x + ((long)PadWidth) * y] = WSymExtension(
            Src, SrcWidth, SrcHeight, x - PadXOffset, y - PadYOffset);
 ...
 */
int ComputePaddedIDFT(num *Dest, int DestWidth, int DestHeight, num *PadTemp,
                      int PadWidth, int PadHeight, numcomplex *Src) {
  ...
  /* Create plan for DFT transform of PadTemp */
  if (!(Plan = FFT(plan_dft_c2r_2d)(PadHeight, PadWidth, Src, PadTemp,
                                    FFTW_ESTIMATE | FFTW_DESTROY_INPUT))) {
    fprintf(stderr, "FFTW plan creation failed.\n");
    return 0;
  }
  ...
  for (y = 0; y < DestHeight; y++, Dest += DestWidth, PadTemp += PadWidth)
    for (x = 0; x < DestWidth; x++) Dest[x] = PadTemp[x];
...
int BlurImage(num *Image, int ImageWidth, int ImageHeight, int NumChannels,
  ...
  while (PadWidth < KernelWidth) PadWidth += 2 * ImageWidth;
  while (PadHeight < KernelHeight) PadHeight += 2 * KernelHeight;
  ...
  if (!(KernelFourier = ComputePaddedDFT(PadTemp, PadWidth, PadHeight, Kernel,
  ...
  for (Channel = 0; Channel < NumChannels; Channel++) {
    ...
    for (y = i = 0; y < PadHeight; y++)
      for (x = 0; x < TransWidth; x++, i++) {
        ...
        ImageFourier[i][1] =
            G[0] * ImageFourier[i][1] + G[1] * ImageFourier[i][0];
        ImageFourier[i][0] = Temp;
      }

    if (!ComputePaddedIDFT(Image + Channel * ImageNumPixels, ImageWidth,
                           ImageHeight, PadTemp, PadWidth, PadHeight,
                           ImageFourier))
      goto Catch;
  }
  ...
  Success = 1;
Catch:
  if (KernelFourier) FFT(free)(KernelFourier);
  if (ImageFourier) FFT(free)(ImageFourier);
  if (PadTemp) FFT(free)(PadTemp);
  return Success;
}
...
void GenerateNoise(num *Data, long NumEl, noisetype NoiseType, num Sigma) {
  ...
  switch (NoiseType) {
    case NOISE_GAUSSIAN:
      for (i = 0; i < NumEl; i++) Data[i] += (num)(Sigma * rand_normal());
      break;
    case NOISE_LAPLACE: {
      const num Mu = (num)(M_1_SQRT2 * Sigma);

      for (i = 0; i < NumEl; i++)
        Data[i] += (num)(rand_exp(Mu) * ((rand_unif() < 0.5) ? -1 : 1));
    } break;
    case NOISE_POISSON: {
      double a, Mean = 0;

      for (i = 0; i < NumEl; i++) Mean += Data[i];
...
int main(int argc, char **argv) {
  ...
  if (!ParseParams(&Params, argc, (const char **)argv)) goto Catch;

  /* Initialize random number generator */
  init_randmt_auto();

  /* Read the input image */
  if (!ReadImageObj(&f, Params.InputFile)) goto Catch;

  /* Perform blurring */
  if (!BlurImage(f.Data, f.Width, f.Height, f.NumChannels, Params.Kernel.Data,
                 Params.Kernel.Width, Params.Kernel.Height))
    goto Catch;

  if (Params.Sigma != 0)
    GenerateNoise(f.Data, (((long)f.Width) * ((long)f.Height)) * f.NumChannels,
...
int ParseParams(programparams *Params, int argc, const char *argv[]) {
  ...
  while (k < argc) {
    Skip = (argv[k][0] == '-') ? 1 : 0;
    kread = CliParseArglist(&Param, &Value, TokenBuf, sizeof(TokenBuf), k,
                            &argv[k][Skip], argc, argv, ":");
    ...
    if (!strcmp(Param, "f") || !strcmp(Param, "input")) {
      ...
      if (!ReadNoise(Params, Value)) return 0;
  ...
  if (!Params->Kernel.Data && !ReadKernel(&Params->Kernel, "disk:0")) return 0;
