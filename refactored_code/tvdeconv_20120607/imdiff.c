...
void MakeDifferenceImage(float *A, const float *B, int Width, int Height,
                         int NumChannels, float D);
void BasicMetrics(float *Max, float *Mse, const float *A, const float *B,
                  int Width, int Height, int NumChannels, int Pad);
float ComputeMssim(const float *A, const float *B, int Width, int Height,
                   int NumChannels, int Pad);
...
void PrintHelpMessage() {
  ...
  printf(
      ...
      "   imdiff [-D <number>] <exact file> <distorted file> <output "
      "file>\n\n");
  printf(
...
int main(int argc, char *argv[]) {
  ...
  if (!(A.Data =
            (float *)ReadImage(&A.Width, &A.Height, Param.FileA,
                               IMAGEIO_FLOAT | IMAGEIO_RGB | IMAGEIO_PLANAR)))
    goto Catch;

  /* Read the distorted image */
  if (!(B.Data =
            (float *)ReadImage(&B.Width, &B.Height, Param.FileB,
                               IMAGEIO_FLOAT | IMAGEIO_RGB | IMAGEIO_PLANAR)))
    goto Catch;
  ...
  if (A.Width != B.Width || A.Height != B.Height) {
    ...
    ErrorMessage("Removal of %d-pixel padding removes entire %dx%d image.\n",
  ...
  if (Param.DifferenceFile) {
    ...
    for (Channel = 0; Channel < 3; Channel++) {
      BasicMetrics(&MaxC[Channel], &MseC[Channel],
                   ...
                   B.Data + Channel * B.Width * B.Height, A.Width, A.Height, 1,
                   Param.Pad);

      if (MaxC[Channel] > Max) Max = MaxC[Channel];
      ...
      case DEFAULT_METRICS:
        if (!Param.SeparateChannels) {
          ...
          printf("Peak signal-to-noise ratio:   %.4f %.4f %.4f\n",
                 -10 * log10(MseC[0]), -10 * log10(MseC[1]),
                 -10 * log10(MseC[2]));
        }

        if (A.Width <= 2 * (5 + Param.Pad) || A.Height <= 2 * (5 + Param.Pad))
          printf("Image size is too small to compute MSSIM.\n");
        else {
          Mssim = (Max == 0) ? 1
                             : ComputeMssim(A.Data, B.Data, A.Width, A.Height,
                                            3, Param.Pad);
...
void MakeDifferenceImage(float *A, const float *B, int Width, int Height,
  ...
  for (n = 0; n < NumEl; n++) A[n] = (A[n] - B[n]) / D + (float)0.5;
}
...
void BasicMetrics(float *Max, float *Mse, const float *A, const float *B,
  ...
  *Max = CurMax;
  *Mse = (float)(AccumMse /
                 (NumChannels * (Width - 2 * Pad) * (Height - 2 * Pad)));
}
...
float ComputeMssim(const float *A, const float *B, int Width, int Height,
  ...
  float *Buffer = NULL, *MuA = NULL, *MuB = NULL, *MuAA = NULL, *MuBB = NULL,
        *MuAB = NULL;
  double MuASqr, MuBSqr, MuAMuB, SigmaASqr, SigmaBSqr, SigmaAB, Mssim = -1;
  int n, x, y, c;
  ...
  SeparableConv2D(MuA, Buffer, A, Window, Window, Boundary, Width, Height,
                  NumChannels);
  SeparableConv2D(MuB, Buffer, B, Window, Window, Boundary, Width, Height,
                  NumChannels);
  ...
  SeparableConv2D(MuAA, Buffer, MuAA, Window, Window, Boundary, Width, Height,
                  NumChannels);
  SeparableConv2D(MuBB, Buffer, MuBB, Window, Window, Boundary, Width, Height,
                  NumChannels);
  SeparableConv2D(MuAB, Buffer, MuAB, Window, Window, Boundary, Width, Height,
                  NumChannels);
  Mssim = 0;
  ...
  for (c = 0; c < NumChannels; c++)
    for (y = Pad; y < Height - Pad; y++)
      for (x = Pad; x < Width - Pad; x++) {
        ...
        Mssim +=
            ((2 * MuAMuB + MSSIM_C1) * (2 * SigmaAB + MSSIM_C2)) /
            ((MuASqr + MuBSqr + MSSIM_C1) * (SigmaASqr + SigmaBSqr + MSSIM_C2));
      }
