...
static int GaussianKernel(image *Kernel, num Sigma) {
  ...
  if (!Kernel || Sigma < 0.0 || !(AllocImageObj(Kernel, W, W, 1))) return 0;
  ...
  if (Sigma == 0.0)
    ...
    for (y = -R; y <= R; y++)
      for (x = -R; x <= R; x++)
        Data[(R + x) + W * (R + y)] = (num)(Data[(R + x) + W * (R + y)] / Sum);
  }
...
static int DiskKernel(image *Kernel, num Radius) {
  ...
  if (!Kernel || Radius < 0.0 || !(AllocImageObj(Kernel, W, W, 1))) return 0;
