...
int main(int argc, char **argv) {
  ...
  if (!TvDeconv(u, f, Params.Kernel, Params.Lambda, Params.Noise)) goto Catch;
...
int TvDeconv(image u, image f, image Kernel, num Lambda, const char *Noise) {
  ...
  memcpy(u.Data, f.Data,
         sizeof(num) * ((size_t)f.Width) * ((size_t)f.Height) * f.NumChannels);
  TvRegSetKernel(Opt, Kernel.Data, Kernel.Width, Kernel.Height);
  TvRegSetLambda(Opt, Lambda);
  TvRegSetMaxIter(Opt, 140);

  if (!(Success =
            TvRestore(u.Data, f.Data, f.Width, f.Height, f.NumChannels, Opt)))
